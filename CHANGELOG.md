...
**功能描述：**
- ✅ 年假由"去年未休不结转"改为**默认逐年累计结转**：自"结转起始年"起，每年发放一份基础年假，上一年未休余额自动结转到下一年累计
- ✅ 系统设置新增"开启年假余额跨年清零"开关（**默认关闭**，即默认结转）；开启后年假按自然年清零、不结转
- ✅ 系统设置新增"年假结转起始年"配置（默认 2025，系统上线年）；上线前已积累的年假可用"年假管理→调整"按生效日期补录为期初
- ✅ 年假管理表格新增"上年结转(天)"列；年假总数 = 基础年假 + 上年结转 + 期初/调整

**计算口径：**
- 结转模式：`剩余 = 基础年假 + 上年结转 + 当年期初/调整 − 当年已用`，逐年向后累计；起始年及更早的期初/已用归入起始年
- 清零模式：`剩余 = 基础年假 + 当年期初/调整 − 当年已用`，按自然年隔离

#### 假期管理新增明细查看

**功能描述：**
- ✅ "调休余额"操作列新增"详情"按钮，弹窗展示主动加班挣得明细、加班调休使用明细、期初/调整明细
- ✅ "年假管理"操作列新增"详情"按钮，弹窗展示年假调休使用明细、期初/调整明细及上年结转
- ✅ 详情/调整/编辑等弹窗支持按 Esc 关闭

**交互优化：**
- ✅ "调休余额"年份筛选默认显示当前年份（原为"累计(全部)"），仍可手动切回累计
- ✅ "被动加班"年份筛选由数字输入框改为下拉框，与其他标签页一致

**技术实现：**
- 后端：新增 `/vacation/comp-leave/detail`、`/vacation/annual-leave/detail` 明细接口，过滤口径与汇总一致
- 后端：`compute_annual_leave` 支持 `yearly_reset` 与 `start_year` 参数实现逐年结转；新增 `annual_leave_yearly_reset`、`annual_leave_start_year` 系统设置
- 前端：年假表格、系统设置、详情弹窗相应更新
