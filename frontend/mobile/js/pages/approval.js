 ...
 * 审批管理页面模块
 */
import { getPendingLeaves, approveLeave } from '../api/leave.js';
import { getPendingOvertimes, approveOvertime } from '../api/overtime.js';
import { formatLeaveDate, formatOvertimeDate } from '../utils/format.js';
...
export async function loadPendingLeaves() {
    try {
        ...
        container.innerHTML = leaves.map(leave => {
            ...
            const dateRange = formatLeaveDate(leave.start_date, leave.end_date);
            
            return `
                <div class="list-item">
                    <div class="item-header">
                        <div class="item-title">${leave.user.real_name || leave.user.username}</div>
                        <span class="status-badge status-pending">${statusName}</span>
                    </div>
                    <div class="item-content">
                        <div class="item-info">${leave.leave_type_name || '请假'}: ${dateRange} 共${leave.days}天</div>
                        <div class="item-reason">${leave.reason || '无'}</div>
                    </div>
                    <div class="item-actions">
                        <button class="btn btn-sm btn-success" onclick="approveLeave(${leave.id}, true)">通过</button>
...
export async function loadPendingOvertimes() {
    try {
        ...
        container.innerHTML = overtimes.map(overtime => {
            ...
            const dateRange = formatOvertimeDate(overtime.start_time, overtime.end_time);
            const typeText = overtime.overtime_type === 'passive' ? '被动加班' : '主动加班';
            
            return `
                <div class="list-item">
                    <div class="item-header">
                        <div class="item-title">${overtime.user.real_name || overtime.user.username}</div>
                        <span class="status-badge status-pending">${statusName}</span>
                    </div>
                    <div class="item-content">
                        <div class="item-info">${typeText}: ${dateRange} 共${overtime.days}天</div>
                        <div class="item-reason">${overtime.reason || '无'}</div>
                    </div>
                    <div class="item-actions">
                        <button class="btn btn-sm btn-success" onclick="approveOvertime(${overtime.id}, true)">通过</button>
 ...
 * 审批请假
 */
export async function approveLeave(leaveId, approved) {
    ...
    try {
        await approveLeave(leaveId, approved, comment || null);
 ...
 * 审批加班
 */
export async function approveOvertime(overtimeId, approved) {
    const action = approved ? '通过' : '拒绝';
    const comment = await showInputDialog(
        `${action}加班申请`,
        `请输入审批意见（可选）`,
    ...
    try {
        await approveOvertime(overtimeId, approved, comment || null);
        await showToast(`审批${action}成功`, 'success');
        await loadPendingOvertimes();
    } catch (error) {
        await showToast(`审批失败: ${error.message}`, 'error');
